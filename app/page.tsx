...
export default function Home(){
 ...
 const tickers=useMemo(()=>new Map(pools.map(p=>[p.pool,p.ticker])),[pools]);
 ...
 const readChain=useCallback(()=>{fetch('./chain.json',{cache:'no-store'})
  .then(r=>r.ok?r.json() as Promise<Chain>:null).then(d=>{if(d)setChain(d)}).catch(()=>{})},[]);
 useEffect(()=>{refresh();readChain();const id=setInterval(()=>{refresh();readChain()},300000);return()=>clearInterval(id)},[refresh,readChain]);
 const t=COPY[lang];
 // The explorer hands back a pool id and nothing else, so the lookup reads the
