...
function compact(n:number){return n>=1e9?`${(n/1e9).toFixed(2)}B`:n>=1e6?`${(n/1e6).toFixed(2)}M`:n>=1e3?`${(n/1e3).toFixed(1)}K`:n.toLocaleString()}
...
/** The six axis scores as ratios of their maximums. This is what every drawing
    on the site is shaped by, at any scale, so it lives in one place. */
const axisRatios=(p:Pool)=>{const q=p.parts||fullParts;
 return[q.reach,q.redundancy,q.independence,q.ownership,q.continuity,q.latency]
  .map((v,i)=>Math.max(0,Math.min(1,(v||0)/AXCAP[i])))};
...
const COPY={
 ja:{
  ...
  legend:[['中心からの距離','ステーク'],['形','そのプールの6軸'],['明るさ','健全性スコア'],['色','プール固有の色相'],['大きさ','委任者']],
 ...
 en:{
  ...
  legend:[['DISTANCE','STAKE'],['FORM','THE POOL’S SIX AXES'],['BRIGHTNESS','HEALTH SCORE'],['COLOUR','THE POOL’S OWN HUE'],['SIZE','DELEGATORS']],
  mote:()=>'GO IN, AND EACH ONE IS THAT POOL’S OWN ARTWORK',
...
/** One pool, drawn.
    ...
    chooses, no pool stands in for the network — which is exactly what putting
    rank 1 in the opening screen used to do, whether or not anyone asked. */
function PoolView({p,lang,motif,setMotif,frame,setFrame}:{
 p:Pool;lang:Lang;motif:number;
 setMotif:React.Dispatch<React.SetStateAction<number>>;
 frame:boolean;setFrame:React.Dispatch<React.SetStateAction<boolean>>;
}){
 const t=COPY[lang],note=(lang==='ja'?MOTIF_NOTE_JA:MOTIF_NOTE_EN)[motif];
 const art=useMemo(()=>artPool(p),[p]),artD=useMemo(()=>artData(p),[p]),
  readout=useMemo(()=>artReadout(p),[p]),evidence=useMemo(()=>axisEvidence(p,lang),[p,lang]),
  pills=useMemo(()=>signalPills(p,lang),[p,lang]);
 const hue={'--pool-hue':`${Math.round(poolHue(p.ticker,p.pool)*360)}deg`} as React.CSSProperties;
 return <section id="pool-view" className={`hero spo-hero${motif===5?' washi':''}`} style={hue}>{motif===5&&<div className="washi-ink" aria-hidden="true"><span>{crestName(poolSig(p.pool))}</span><b>{p.ticker||'無銘'}</b></div>}<div className="copy web3-copy"><div className="chain-state"><span>● CARDANO MAINNET</span><span>POOL RANK #{p.rank}</span><span>OBSERVED {p.checked||'—'}</span></div>{motif===5
     ?<div className="kamon-emblem"><i/><span>JAPAN</span><b>紋</b><span>KAMON</span><i/><em>{t.emblem}</em></div>
     :<p className="eyebrow">{t.eyebrow}</p>}<h1><span>{p.pool.slice(0,16)}…</span>{p.ticker||'UNTITLED'}<br/>ALIVE.</h1><p className="hashline">{p.pool}</p><p className="intro">{t.intro[0]}<br/>{t.intro[1]}</p><div className="readout">
    <div className="rd-figures">
     <div><b>{compact(p.stake)} ₳</b><span>{t.stake}</span></div>
     <div><b>{p.delegators.toLocaleString()}</b><span>{t.delegators}</span></div>
     <div><b>{p.blocks.toLocaleString()}</b><span>{t.blocks}</span></div>
     <div><b>{(p.sat*100).toFixed(1)}%</b><span>{t.sat}</span></div>
    </div>
    <div className="rd-health">
     <div className="rd-score"><b>{p.score}</b><i>/100</i><span>{t.health}</span>
      <small>{(p.margin*100).toFixed(2)}% + {p.fixedAda} ₳ {t.fee}</small></div>
    ...
    <p className="rd-desc">{note[1]}</p>{motif===5&&<p className="rd-meaning"><span>{t.meaning}</span>{crestMeaning(poolSig(p.pool),lang)}</p>}
   ...
   <div className={`hero-art${frame?' framed':''}`} style={hue}><button className="frame-toggle" onClick={()=>setFrame(f=>!f)} title="Toggle frame">{frame?'FRAME ON':'FRAME OFF'}</button><div className="motif-switch">{SHOWN_MOTIFS.map((m,i)=><button key={m} className={i===motif?'on':''} onClick={()=>setMotif(i)}>{m}</button>)}</div><div className="art-color" style={{filter:`saturate(${1.05+Math.min(.55,p.sat)})`}}>{motif===0?<BlockArt block={art} large/>:<PoolArt data={artD} motif={motif}/>}</div>{frame&&<div className="art-plate">{motif===5
     ?<><i className="plate-seal">紋</i><b className="plate-mon">{crestName(poolSig(p.pool))}</b><span className="plate-romaji">{crestRomaji(poolSig(p.pool))}</span><code>{p.ticker||'無銘'} · {p.pool.slice(0,14)}…</code></>
     :<><b>{p.ticker||'UNTITLED'}</b><span>RANK #{p.rank}</span><span>HEALTH {p.score}/100</span><code>{p.pool.slice(0,22)}…</code></>}</div>}{motif!==5&&<div className="art-caption"><span>IDENTITY = FORM</span><span>HEALTH = FLOW</span><span>HISTORY = MEMORY</span></div>}</div>
</section>;
}
...
export default function Home(){
 ...
 // A refresh keeps whatever the reader was already looking at, and otherwise
 // keeps nothing. It used to fall through to next[0], which is how rank 1 came
 // to be selected every time without anyone choosing it.
 const refresh=useCallback(async()=>{try{const next=await loadPools();setPools(next);setSelected(x=>x?next.find(p=>p.pool===x.pool)||x:null);setStatus('live')}catch(e){console.error(e);setStatus('demo')}},[]);
 useEffect(()=>{refresh();const id=setInterval(refresh,300000);return()=>clearInterval(id)},[refresh]);
 const t=COPY[lang];
 // The disc is built once and keeps its own callback, so the lookup reads the
 // current list through a ref rather than re-registering on every refresh.
 ...
 /* What the disc has settled on. Past a certain magnification the motes can no
    longer stand in for the pool's artwork - there are not enough of them, and
    padding every pool out to where they would be is hundreds of thousands of
    points nobody is looking at - so the real drawing is put in their place. */
 ...
 const filtered=useMemo(()=>{const q=query.trim().toLowerCase();return q?pools.filter(p=>(p.ticker||'').toLowerCase().includes(q)||p.pool.toLowerCase().includes(q)):pools},[pools,query]);
 // Choosing scrolls to the pool that was chosen. On the first choice that
 // section does not exist yet, so the scroll waits for it to be painted.
 ...
 return <main><nav><div className="brand">SPO / <span>ALIVE</span></div><div className="nav-right"><div className="lang-switch">{(["ja","en"] as Lang[]).map(l=><button key={l} className={l===lang?"on":""} onClick={()=>setLang(l)}>{l==="ja"?"日本語":"EN"}</button>)}</div><div className={`status ${status}`}><i/>{status==='live'?'RELAY HEALTH · LIVE':status==='loading'?'CONNECTING':'CACHED SAMPLE'}</div></div></nav>
  ...
  <section className="galaxy-hero">
   ...
   <div className="galaxy-copy" ref={copyRef}>
    ...
    <h1><span>{t.netKicker}</span>CARDANO BLOCKCHAIN<br/>ALIVE.</h1>
   ...
   <a className="scroll-cue" href="#select"><span>{t.scroll}</span><i/></a>
  </section>

  <section id="select" className="timeline pool-stream" aria-label="SPO selector"><div className="select-head"><h2>{t.selectHead}</h2><p>{t.selectNote}</p></div><div className="timeline-head"><span>← STRONG RELAY HEALTH</span><span>{t.scroll}</span><span>NETWORK DIVERSITY →</span></div><div className="rail pool-rail">{pools.slice(0,12).map(p=><button key={p.pool} className={`block pool-card ${p.pool===selected?.pool?'active':''}`} onClick={()=>choose(p)}><div className="pool-card-top"><span>#{p.rank}</span><i className={`severity-${p.severity}`}/><b>{p.ticker||'—'}</b></div><div className="pool-stake"><strong>{compact(p.stake)} ₳</strong><span>{(p.sat*100).toFixed(1)}% SAT</span></div><div className="pool-bar"><span style={{width:`${Math.max(2,Math.min(100,p.sat*100))}%`}}/></div><dl><div><dt>HEALTH</dt><dd>{p.score}</dd></div><div><dt>HOSTS</dt><dd>{p.reachable}/{hosts(p)}</dd></div><div><dt>RTT</dt><dd>{p.rtt==null?'—':`${p.rtt.toFixed(0)}ms`}</dd></div><div><dt>BLOCKS</dt><dd>{p.blocks}</dd></div></dl></button>)}</div></section>

  {selected&&<PoolView p={selected} lang={lang} motif={motif} setMotif={setMotif} frame={frame} setFrame={setFrame}/>}

  <section className="pool-directory" aria-label="All stake pools"><div className="directory-head"><div><p>NETWORK DIRECTORY</p><h2>Every pool,<br/>a living artwork.</h2></div><label><span>SEARCH TICKER / POOL ID</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search all observed pools…"/><small>{filtered.length.toLocaleString()} / {pools.length.toLocaleString()} POOLS</small></label></div><div className="directory-columns"><span>RANK / POOL</span><span>HEALTH</span><span>HOSTS</span><span>STAKE / SAT</span><span>RTT</span><span>BLOCKS</span></div><div className="directory-list">{filtered.map(p=><button key={p.pool} className={p.pool===selected?.pool?'selected':''} onClick={()=>choose(p)}><div className="dir-identity"><em>#{p.rank}</em><i className={`severity-${p.severity}`}/><strong>{p.ticker||'UNTITLED'}</strong><code>{p.pool}</code></div><b>{p.score}<small>/100</small></b><span>{p.reachable}/{hosts(p)}<small> HOSTS</small></span><span>{compact(p.stake)} ₳<small>{(p.sat*100).toFixed(1)}% SAT</small></span><span>{p.rtt==null?'—':`${p.rtt.toFixed(0)} ms`}</span><span>{p.blocks.toLocaleString()}</span></button>)}</div></section>
